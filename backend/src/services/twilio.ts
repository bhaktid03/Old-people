...
export async function sendOtpSms(toPhoneE164: string, message: string): Promise<void> {
  const messagingServiceSid = process.env.TWILIO_MESSAGING_SERVICE_SID;
  const fromNumber = process.env.TWILIO_FROM_NUMBER; // alternative to messaging service

  const sendParams: Record<string, string> = { to: toPhoneE164, body: message } as const as any;
  if (messagingServiceSid) {
    (sendParams as any).messagingServiceSid = messagingServiceSid;
  } else if (fromNumber) {
    (sendParams as any).from = fromNumber;
  } else {
  ...
  try {
    await client.messages.create(sendParams as any);
  } catch (err: any) {
